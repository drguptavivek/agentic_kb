---
title: Keycloak Theme Variants and Realm Branding (VG Pattern)
type: howto
domain: Keycloak
tags:
  - keycloak
...
# Keycloak Theme Variants and Realm Branding (VG Pattern)

## Goal

Maintain separate branded themes per realm (example: `vg` and `vg-master`) and apply them safely using `kcadm`.
...
```bash
./kcadm.sh update realms/org-new-delhi -s loginTheme=vg -s adminTheme=vg -s accountTheme=vg --config .kcadm.config
./kcadm.sh update realms/master -s loginTheme=vg-master -s adminTheme=vg-master -s accountTheme=vg-master --config .kcadm.config
```
...
```bash
chmod 644 keycloak-26.5.3/themes/vg-master/login/resources/img/logo.png
```
...
```bash
rsync -a --delete keycloak-26.5.3/themes/vg/ theme/vg/
rsync -a --delete keycloak-26.5.3/themes/vg-master/ theme/vg-master/
```

Source -> runtime:

```bash
rsync -a --delete theme/vg/ keycloak-26.5.3/themes/vg/
rsync -a --delete theme/vg-master/ keycloak-26.5.3/themes/vg-master/
```
