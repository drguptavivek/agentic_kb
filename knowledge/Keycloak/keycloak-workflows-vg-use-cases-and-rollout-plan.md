---
title: Keycloak Workflows Use Cases and Rollout Plan for VG SSO
type: howto
domain: Keycloak
tags:
  - keycloak
...
# Keycloak Workflows Use Cases and Rollout Plan for VG SSO

## Overview

This note maps Keycloak Workflows capabilities to practical use cases for the `org-new-delhi` realm and provides a rollout sequence that fits this repository.
...
```yaml
name: VG onboarding baseline
on: user_created
if: not has-role('realm-admin')
steps:
  - uses: notify-user
    with:
      subject: "Welcome to VG SSO"
      message: |
...
steps:
  - uses: notify-user
    after: 180d
    with:
      subject: "VG account inactivity notice"
      message: "Please sign in to keep your account active."
  - uses: notify-user
    after: 60d
    with:
      subject: "VG account final warning"
...
```yaml
name: VG contractor account review
schedule:
  after: 24h
  batch-size: 100
