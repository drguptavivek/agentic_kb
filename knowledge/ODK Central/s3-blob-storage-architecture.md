...
# ODK Central S3 Blob Storage Architecture
...
services:
  garage:
    ...
    ports:
      - "127.0.0.1:3900:3900"  # S3 API - localhost only for security
      - "127.0.0.1:3903:3903"  # Web UI - localhost only for security
    volumes:
      - garage_data:/data
    environment:
      - RUST_LOG=info
...
**Security Note**: Ports bound to `127.0.0.1` are only accessible from the host machine. See "Security" section below for production recommendations.
...
### Step 5: Restart ODK Central Service
...
## Security: Protecting Garage Web UI

### Local Development

For local development, the quickstart binds Garage to `127.0.0.1`:

```yaml
ports:
  - "127.0.0.1:3900:3900"  # S3 API - localhost only
  - "127.0.0.1:3903:3903"  # Web UI - localhost only
```

This prevents external access. Only the host machine can access these ports.

### Production Deployment

For production, **use network/firewall rules** to restrict access to Garage. You may use Host Firewall (UFW/Iptables etc), local network firewall in the infrastructure or Cloud firewall of hosting provider (Digitalocean / AWS / GCP / Azure etc).

For additional security, put Nginx in front of Garage:
Use Allow From directoves in NGINX to only allow access from your trusted IPs/Networks.

```nginx
location /garage/ {
    auth_basic "Garage Admin";
...
Only expose Garagebehind a VPN or reverse proxy with authentication.
